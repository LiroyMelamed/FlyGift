...
export type DateFieldProps = FieldBase &
    Omit<InputHTMLAttributes<HTMLInputElement>, "className" | "type" | "onChange" | "value"> & {
        className?: string;
        value: string;
...
export const DateField = forwardRef<HTMLInputElement, DateFieldProps>(
    function DateField(
        { label, hint, error, warning, className, id, value, onChange, ...rest },
        ref
    ) {
        const inputId = id || `f-${label.toLowerCase().replace(/\s+/g, "-")}`;
        return (
            <div className={cn("min-w-0 space-y-1.5", className)}>
                ...
                <input
                    ...
                    {...rest}
                    className={cn(
                        "date-input w-full min-w-0 max-w-full rounded-xl border border-[#0F172A]/20 dark:border-white/10",
                        "bg-white/70 dark:bg-white/[0.04] px-3 py-3",
                        "text-base text-[#0F172A] dark:text-text-primary",
                        "transition-all focus:outline-none focus:border-cyan-jet/60 focus:bg-white/90 dark:focus:bg-white/[0.06]",
                        "focus:shadow-[0_0_0_4px_rgba(0,229,255,0.12)]",
                        error && "border-danger/60"
                    )}
                ...
                {(hint || error || warning) && (
                    <p
                        className={cn(
                            "text-xs",
                            error ? "text-danger" : warning ? "text-warning" : "text-text-secondary"
                        )}
                    >
                        {error || warning || hint}
                    </p>
